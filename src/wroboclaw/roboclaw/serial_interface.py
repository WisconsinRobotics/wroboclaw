...
class SerialCommandHandler:
    ...
    def invoke(self, *args: Any) -> bool:
        ...
        rospy.loginfo(f"Header data:  {self._header}")
        #rospy.loginfo(f"Write Status (Header):  {self._serial.write(self._header)}")
        rospy.loginfo(f"Data data:  {data}")
        #rospy.loginfo(f"Write Status (Data):  {self._serial.write(data)}")
        rospy.loginfo(f"Write Status (CRC):  {self._serial.write(self._header+data+STRUCT_CRC16.pack(csum & 0xFFFF))}")
